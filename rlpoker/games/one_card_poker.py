...
class OneCardPoker(extensive_game.ExtensiveGame):
    ...
    def compute_utility(betting_actions, hole_cards):
        ...
        # We now change this so that playing 0 to a 1 means you fold the hand
        # and do not win the pot.
        if betting_actions[-2:] == [1, 0]:
            # In this situation, the player who bet 1 wins.
            winner = 1 if len(betting_actions) % 2 == 0 else 2
            loser = 1 if winner == 2 else 2
        else:
            # Otherwise it goes to a showdown.
