...
class DeepRegretNetwork(RegretPredictor):
    ...
    def __init__(self, state_dim: int, action_indexer: neural_game.ActionIndexer, player: int):
        ...
        self.tensors, self.init_op = self.build(self.state_dim, self.action_indexer.action_dim, self.scope)
...
def deep_cfr(game: extensive_game.ExtensiveGame, action_indexer: neural_game.ActionIndexer,
             ...
             num_iters: int=100, num_traversals: int=50,
    ...
    with tf.Session() as sess:
        ...
        network2 = DeepRegretNetwork(game.state_dim, action_indexer, 2)
...
def cfr_traverse(game: extensive_game.ExtensiveGame, action_indexer: neural_game.ActionIndexer,
    ...
    """
    print("cfr_traverse called with node: {}".format(node))
    if is_terminal(node):
        print("Is terminal, and payoffs are: {}".format(payoffs(node)))
